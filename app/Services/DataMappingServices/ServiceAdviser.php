...
class ServiceAdviser extends MonthlyDataMapping {
    ...
    protected function metricsMapping($row): array {
        return [
            ...
            'sos_navara_result' => $row['score_ce_SOS3_N'] !== '' ? number_format(floatval($row['score_ce_SOS3_N']), 1) : 0,
            'sos_patrol' => $row['points_ce_SOS3_P'] !== '' ? intval($row['points_ce_SOS3_P']) : 0,
            'sos_patrol_result' => $row['score_ce_SOS3_P'] !== '' ? number_format(floatval($row['score_ce_SOS3_P']), 1) : 0,

            'fft' => $row['points_ce_FFT3_'] !== '' ? intval($row['points_ce_FFT3_']) : 0,
            'fft_result' => $row['score_ce_FFT3_'] !== '' ? number_format(floatval($row['score_ce_FFT3_']), 1) : 0,

            'cwp' => $row['points_ce_cwp3'] !== '' ? intval($row['points_ce_cwp3']) : 0,
            'cwp_result' => $row['score_ce_cwp3'] !== '' ? number_format(floatval($row['score_ce_cwp3']), 1) : 0,

            'eoc' => $row['points_ce_EOC3_'] !== '' ? intval($row['points_ce_EOC3_']) : 0,
            'eoc_result' => $row['score_ce_EOC3_'] !== '' ? number_format(floatval($row['score_ce_EOC3_']), 1) : 0,
            ...
            'loyalty_result' => $row['sales_loyalty'] !== '' ? number_format(floatval($row['sales_loyalty']), 1) : 0,
        ];
    }
}
